...
const APP = {
  ...
  inlineStyleAllow: {
    ...
    confetti: 'מיקום כל פתית נדגם באקראי בזמן ריצה — תשעים אתרים שאין להם '
      + 'שני מופעים זהים, ⛔ ומחלקה לכל אחד הייתה מחלקה לכל הרצה',
  },
  inlineColorAllow: {},
  ...
  writeActs: ['entry-menu', 'order-save'],
  writeActsWhy: '',
  ...
  gapRows: [46, 77, 125, 127, 131, 155, 165, 140, 92, 68, 69, 70, 122, 156, 171, 172, 173, 174, 175, 176, 177, 178, 180],
  ...
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    176: (c) => c.hasCode(/G_PASS_ITER\s*=\s*100000\b/)
  ...
  migrateKeep: {},
  mirrorTables: ['kp_years', 'kp_months', 'kp_standing_orders', 'kp_so_instances', 'kp_entries', 'kp_lookups'],
  ...
  enterFields: ['f-amount', 'f-desc', 'f-date', 'o-amt'],
  enterExempt: [],
  ksaveActs: ['flow-next', 'order-save'],
  elemListeners: [
...
const FLOOR = { shared: 84, app: 6, appWhy: 'שכבת המראה ושני בלוקי גרף העמודות; ומעליהם שער מצב הרשת ומנוע התאריך העברי — ⛔ ואין כאן שכבת כניסה' };
/* ⚠️ סוף פר-אפליקציה */
