...
const APP = {
   ...
   *  רשומה ⛔ ולא לסחיפה. */
  appTokens: {
    '--shadow-brand': 'הצל הצבוע של כפתור ההוספה הצף — מילוי הצל נגזר מגוון הזהות, ואין בשאר כפתור צף שצילו צבוע',
    '--border-2': 'קו גבול משני לטבלאות התנועות — דרגה שנייה למושג הגבול, ומוצר',
    '--brand-fill': 'המילוי של גוון הזהות בכפתורים, ו-`--brand` נגזר ממנו — מוצר ולא תשתית',
    '--brand-ink': 'דיו גוון הזהות — תפקיד שני לאותו גוון, ולכן אסימון נפרד',
    '--brand-soft': 'שטיפת גוון הזהות מאחורי הכרטיס — מוצר ולא תשתית',
    '--gold': 'דיו ההתחייבות שנגבתה במלואה — הישג של הגיוס, ואינו מצב של פעולה',
    '--gold-soft': 'המילוי שמתחת לדיו ההתחייבות — משטח ולא דיו, ומוצר ולא תשתית',
   ...
   *  ⚠️ ⛔ משווה אחד בשם — ⚠️ `state.donors` הוא הרשימה היחידה שסדרה מוצהר: ⭐ השם נבדל ביניהן
  ...
  writeActs: ['login', 'switch-confirm', 'donor-save', 'donor-del', 'vip-toggle', 'notes-save',
              'txn-save', 'txn-del', 'pledge-save', 'pledge-del', 'task-save', 'task-del',
              'user-save', 'user-toggle', 'pass-save', 'my-pass-save', 'target-save',
              'cfg-add', 'cfg-edit-save', 'cfg-del', 'dp-n-save'],
  writeActsWhy: '',
  ...
  gapRows: [45, 75, 122, 124, 128, 152, 162, 61, 137, 90],
  ...
  inlineErrAllow: {
    'lg-err': 'הודעת הכניסה — ⛔ נשארת על המסך בזמן שהמשתמש מקליד מחדש: ⚠️ טוסט שנעלם משאיר טופס כניסה בלי הסבר',
  },
  orphanAllow: {
  ...
  contrastAllow: {
    '.card > h2 .cnt|בהיר': 'מונה משני על קו מפריד — ⚠️ גוון קיים ⛔ ולא תוצאה של הפיצול: ⭐ שינויו הוא החלטת מנהל',
    '.kcol > h3 .cnt|בהיר': 'מונה משני על קו מפריד — ⚠️ גוון קיים ⛔ ולא תוצאה של הפיצול: ⭐ שינויו הוא החלטת מנהל',
    '.badge|בהיר': 'תווית משנית על קו מפריד — ⚠️ גוון קיים ⛔ ולא תוצאה של הפיצול',
    'th|בהיר': 'כותרת עמודה משנית — ⚠️ גוון קיים ⛔ ולא תוצאה של הפיצול',
  },
  tableProbe: {
  ...
  migrateKeep: {},
  mirrorTables: ['g_donors', 'g_pledges', 'g_txns', 'g_tasks', 'g_targets', 'g_users', 'g_config'],
  /*  ⛔ אין כאן מפתח `ls*` בשם מפורש שאינו טבלה — ⚠️ **וההיעדר מוצהר
   *  ריק** ⛔ ואינו נשמט: ⭐ שדה חסר נקרא «לא נשאל», וריק «נמדד ואין». */
  flatKeys: {
    g_sw_dismissed: 'סימן דחיית באנר העדכון — נושא את שם המטמון שנדחה, ואינו בענן כלל',
  },
  ...
  enterFields: ["cfg-add-' + c.key + '", 'txn-amount', 'txn-date', 'txn-agent',
                'pl-amount', 'pl-due', 'pl-agent', 'tk-title', 'tk-assignee', 'tk-due',
                'dn-name', 'dn-phone', 'dn-agent', 'dn-tags', 'us-full', 'us-name',
                'us-pass', 'pw-new', 'mp-cur', 'mp-new', 'mp-new2', 'sw-pass',
                'tg-month', 'tg-amount', 'cfg-v', 'חיפוש תורם לפי שם או טלפון',
                '.inp dp-n-name', '.inp dp-n-phone', 'lg-user', 'lg-pass'],
  enterExempt: [
    { field: 'donor-q', group: 'no-save', why: 'חיפוש תורם — אין בשורה כפתור שמירה' },
    { field: 'pf-from', group: 'no-save', why: 'סינון התחייבויות — אין בכרטיס כפתור שמירה' },
    { field: 'pf-to', group: 'no-save', why: 'סינון התחייבויות — אין בכרטיס כפתור שמירה' },
  ],
  ksaveActs: ['cfg-add', 'txn-save', 'pledge-save', 'task-save', 'donor-save',
              'user-save', 'pass-save', 'my-pass-save', 'switch-confirm',
              'target-save', 'cfg-edit-save', 'dp-pick', 'dp-new', 'dp-n-save',
              'login'],
  elemListeners: [
    { host: 'c', why: 'כרטיס הקנבן הנגרר — `dragstart`/`dragend` נרשמים על הכרטיס עצמו' },
    { host: 'col', why: 'עמודת הקנבן כיעד הגרירה — `dragover`/`drop` נרשמים על העמודה עצמה' },
  ],
  docListeners: [
