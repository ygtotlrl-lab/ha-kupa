...
const APP = {
   ...
   *  רשומה ⛔ ולא לסחיפה. */
  appTokens: {
    '--brand-ink': 'דיו גוון הזהות — תפקיד שני לאותו גוון, ולכן אסימון נפרד',
    '--brand-soft': 'שטיפת גוון הזהות מאחורי הכרטיס — מוצר ולא תשתית',
    '--on-rasp': 'הדיו שעל הכפתור הצף — ⚠️ צמד הפוך ל-`--rasp` שמתחלף איתו יחד, ⭐ ואין בשאר כפתור צף צבוע',
    '--ring-soft': 'קו הטבעת שעל המטבע — ⚠️ דיו שקוף למחצה על משטח צבעוני, ⭐ ואין בשאר מטבע שנגרר',
    '--radius-sm': 'רדיוס התגית הקטנה — נגזר מסולם הרדיוס, ואין תגית כזו בשאר',
    '--shadow-lg': 'הצל העמוק של המודאל — דרגה שנייה למושג הצל, ומוצר ולא תשתית',
    '--purple-1': 'המשטח הרך של הסגול — רקע האייקון והשלד, ואין בשאר סולם גוון פר-תפקיד',
    '--purple-2': 'הסגול הבהיר של הפס המשני — דרגה בסולם הגוון, ואין בשאר סולם כזה',
    '--purple-3': 'סגול הזהות המלא — גוף המטבע והפתיתים, ואין בשאר סולם כזה',
    '--purple-4': 'הסגול הבינוני של הקצה — דרגה בסולם הגוון, ואין בשאר סולם כזה',
    '--purple-5': 'דיו הניווט והחיצים — דרגה בסולם הגוון, ואין בשאר סולם כזה',
    '--purple-6': 'דיו האייקון על משטח רך — הדרגה הכהה בסולם, ואין בשאר סולם כזה',
    '--purple-7': 'הסגול העמוק של הרקע הכהה — קצה הסולם, ואין בשאר סולם כזה',
    '--peach-1': 'המשטח הרך של האפרסק — רקע שורת החוב והאייקון שלה, ואין בשאר סולם פר-תפקיד',
    '--peach-2': 'האפרסק המלא של הפס — דרגה בסולם הגוון, ואין בשאר סולם כזה',
    '--rasp': 'מילוי הכפתור הצף — סימן שמופיע בשני מקומות בלבד, ואין בשאר כפתור צף צבוע',
    '--rasp-ink': 'דיו שורת הפתיחה — אותו גוון בתפקיד הדיו, ⛔ ואסימון נפרד: ⚠️ אסימון נושא תפקיד אחד',
    '--line-soft': 'קו מפריד רך בין פריטי הפירוט — דרגה שנייה למושג הקו, ומוצר',
  },
  /*  ⛔ וארבע יכולות שכבת הכניסה נעדרות כאן אף הן — ⚠️ שני משתמשים
   *  ונתונים משותפים, ⛔ ואין מסך כניסה ואין טבלת משתמשים: ⭐ ולכן אין
   *  סשן שיוחזק, אין מה לנעול, אין תפקיד שיוכרע, ואין כתיבת משתמש
   *  שתעבור בפונקציה אחת. ⛔ יכולת שאין לה קורא היא קוד מת. */
  skipCaps: ['writeUser', 'lock', 'sess', 'isAdmin'],
   ...
   *  השורה: `id` לרוב הטבלאות, ⛔ `key` ל-`g_config` ו-`month` ל-`g_targets`. */
  conflictKeys: { client_id: 'מזהה שנוצר במכשיר', id: 'מפתח הטבלה', key: 'הגדרה לפי מפתח', month: 'יעד חודשי יחיד' },
  /*  ⛔ נקודת המעבר היחידה של סוד אל הדיסק (סבב 76) — ⚠️ השם נבדל בין
   *  השלוש והמנגנון אחד: ⭐ כאן רשימת עמודות **נשללות** שמוצהרת על הטבלה,
   *  ⛔ ושני נתיבי הכתיבה — המלא והחלקי — עוברים בה. */
  secretStripFn: 'usersSanitize',
  /*  ⛔ השדה שאסור לו לשרוד את נקודת המעבר — ⚠️ תשובת הכניסה מכילה אותו
   *  בוודאות, ⛔ ולכן הנתיב החלקי הוא בדיוק ההזדמנות לדלוף. */
  /*  ⛔ אין כאן טבלת משתמשים ואין סוד בשורה — ⚠️ **וההיעדר מוצהר** ⛔ ואינו
   *  נשמט: ⭐ שדה חסר נקרא «לא נשאל», וריק נקרא «נמדד ואין». */
  secretField: null,
  /*  ⛔ אין כאן מסלול השלמת טביעה (סבב 76) — ⚠️ כתיבת משתמש דורשת רשת
   *  ועוברת בגזירת הטביעה, ⛔ ולכן משתמש בלי טביעה אינו נוצר כלל: ⭐ מסלול
   *  השלמה כאן היה קוד מת שהשער מאשר. */
  passFpFillFn: null,
  userCreateFn: 'saveUser',
  passFpMakeFn: 'passFields',
  /*  ⛔ נקודת המיון היחידה, ⛔ ושדות הסדר שמזהים משווה של ישות —
   *  ⚠️ הרישומים הם הרשימה היחידה שסדרה מוצהר, ⭐ והוא תאריך ואז מזהה:
   *  ⛔ השם נבדל בין האפליקציות ⛔ והמנגנון אחד, ⚠️ ולכן ההצהרה כאן
   *  וה-probe משותף. */
  sortFn: 'kSortEntries',
  sortFields: ['entry_date'],
  ...
  cloudPut: ['cfgWrite'],
  cloudPutWhy: '',
  cloudWrites: ['cfgWrite'],
  /*  ⛔ עוטף שאינו נקודת מעבר בעצמו — ⚠️ **מה נכנס**: שם הפונקציה
   *  ולמה הכתיבה שבה אינה כתיבה לענן מצידה, ⛔ **ומה מפיל**: הצהרה
   *  שאין לה אתר. ⭐ **ולמה המבנה קיים**: קידום האות יושב בנקודת המעבר
   *  האחת, ⛔ וקידום שני בעוטף היה מקדם את האות פעמיים לכתיבה אחת. */
  cloudWriteAllow: {
    lookupSeed: 'מממשת את ברירות המחדל לשורות ומקדמת את עידן הרשימות דרך `cfgWrite` — ⛔ והיא אינה נקודת מעבר בעצמה: ⚠️ `cfgWrite` הוא שכותב לענן, ⭐ והוא שמקדם את האות',
    lookupReorder: 'כותבת את הסדר החדש ומקדמת את עידן הרשימות דרך `cfgWrite` — ⛔ והיא אינה נקודת מעבר בעצמה: ⚠️ `cfgWrite` הוא שכותב לענן, ⭐ והוא שמקדם את האות',
  },
  ...
  actMap: 'DOM_ACTIONS',
  /*  ⛔ מה שכתיבתו מחליפה הקשר — ⚠️ כאן זה המשתמש המחובר: ⭐ `state.user`
   *  הוא חלון אל מודול הסשן, ⛔ והניקוי עובר בקובע שלו. */
  ctxKeys: { 'state.user': 'המשתמש המחובר — ⛔ כל כתיבה אליו היא החלפת הקשר',
  ...
  ctxCycles: ['syncPull', 'syncNow'],
  ctxRegs: ['pendConfirmPush', '_gPushedAt'],
  ...
  writeActs: ['entry-del', 'entry-save', 'order-new', 'order-save', 'pledge-save'],
  writeActsWhy: '',
  ...
  scanKind: {
    ...
    'test_sums': 'whiten',
    'test_chumash': 'whiten',
    'test_so_instances': 'raw — הנמדד הוא **איזה ליטרל** יושב ברשומת המופע, וההלבנה מוחקת בדיוק את מה שהוא מודד',
    'test_so_version': 'whiten',
    'test_rasp': 'raw — הערך החזותי יושב גם בתוך מחרוזת CSS שנכתבת מ-JS, וההלבנה מוחקת את מה שהוא מודד',
  ...
  gapRows: [226, 81, 138, 140, 144, 168, 179, 153, 99, 72, 73, 74, 135, 169, 186, 187, 188, 189, 190, 191, 192, 193, 195],
  ...
  rowsLayerFns: {},
  fullPullAllow: {},
  /*  ⛔ אין כאן רשומה שנושאת מערך פריטים — ⚠️ `g_config` היא טבלת
   *  `{key, value}` שממוזגת פר-מפתח, ⭐ ואין בה פריטים בעלי מזהה ·
   *  ⛔ **וההיעדר מוצהר ריק** ⛔ ואינו נשמט. */
  mergePoints: [],
  ...
  /*  ⛔ אין כאן אזור שגיאה מוטבע — ⚠️ **וההיעדר מוצהר ריק** ⛔ ואינו
   *  נשמט: ⭐ שדה חסר נקרא «לא נשאל», וריק נקרא «נמדד ואין». */
  inlineErrAllow: {},
  orphanAllow: {
  ...
  /*  ⛔ אין כאן צמד שאינו עומד ב-4.5 — ⚠️ **וההיעדר מוצהר ריק** ⛔ ואינו
   *  נשמט: ⭐ שדה חסר נקרא «לא נשאל», וריק נקרא «נמדד ואין». */
  contrastAllow: {},
  tableProbe: {
     ...
     *  ניתנת למדידה במלואה. */
    191: (c) => c.hasCode(/G_PASS_ITER\s*=\s*100000\b/)
          && /iterations:\s*G_PASS_ITER,\s*hash:\s*'SHA-256'/.test(c.src)
          && /deriveBits\([\s\S]{0,240}?key,\s*256\)/.test(c.code),
    ...
    190: (c) => !!c.fnBody('usersCacheSave')
             && (c.code.match(/\busersCacheSave\s*\(/g) || []).length > 1,
    // טבלאות `g_` שב-`PUSH_TABLES` מאז ההקמה — המראה נבנית מהן ישירות.
    151: (c) => c.hasCode(/\bPUSH_TABLES\b/),
    /*  ⛔ נקודת כניסה חיה ולא קיום פונקציה (סבב 67) —
     *  ⚠️ המסך נבנה כמחרוזת, ולכן העדות היא `data-act` שמנתב אליו
     *  **וגם** שלושת השדות שהוא קורא מהם. */
    187: (c) => !!c.fnBody('formMyPassword')
  ...
  gateKind: {
    ...
    'test_chumash':        'text',
    'test_pledge':       'text',
    'test_so_instances':   'text',
    'test_so_version':     'text',
    'test_rasp':           'text',
   ...
   *  ⛔ ומפתח שאינו נדחף ואינו מוכרז ב-`MIRROR_CFG.noPush` — ⚠️ וכאן
   *  `g_users` היא היחידה, ⭐ שמראתה חיה בשכבה ואינה נדחפת. */
  ...
  migrateKeep: {
    kMigrateLegacyPrefix: 'הגירת התחילית מ-`kp_` ל-`k_` (סבב 148) — ⚠️ התור שלא נדחף וזהות המכשיר נושאים במכשיר שלא נטען מאז את השם הישן, ⛔ ולתור אין מקור שני בענן: ⭐ הסרתה מאבדת אותו לנצח',
  },
  ...
  enterFields: ['f-amount', 'f-desc', 'f-date', 'o-amt', 'o-name', 'o-day',
                'e-amt', 'e-desc', 'e-date', 'p-amt', 'l-label'],
  enterExempt: [],
  ksaveActs: ['flow-next', 'entry-save', 'order-new', 'order-save', 'pledge-save', 'look-save'],
  ...
  elemListeners: [
    { host: 'c', why: 'המטבע הנגרר — `pointerdown`/`pointermove`/`pointerup` נרשמים על המטבע עצמו, שהגרירה היא רצף אירועים על אלמנט אחד' },
  ],
  /*  ⛔ ריק ומוצהר — ⚠️ כל מאזין גלובלי כאן נושא ארגומנט שהוא מחרוזת,
   *  ⭐ ואחד לכל אירוע: ⛔ והרשימה הזו היא למי שארגומנטו אינו מחרוזת.
   *  ⚠️ **ושלושת מאזיני הגרירה** — `pointerdown` · `pointermove` ·
   *  `pointerup` — ⭐ הם אירוע אחד לכל אחד, ⛔ ואינם מאזין שני לאירוע
   *  קיים: ⚠️ ושומר המגע שבבאנר העדכון יושב על `window` ⛔ ולא על
   *  `document`. */
  docListeners: [],
  actExempt: [],
  keydownExempt: [
...
const FLOOR = { shared: 87, app: 5, appWhy: 'שכבת המראה ושני בלוקי גרף העמודות; ומעליהם מנוע התאריך העברי — ⛔ ואין כאן שכבת כניסה' };
/* ⚠️ סוף פר-אפליקציה */
...
/*  ⭐ סבב 52 — נעילת חוסר-פעילות. ה-probe דורש את **שני** התנאים:
  ...
  { row: 187, name: 'שכבת כניסה מלאה',
    /*  ⛔ ההיקף הוא מפת הפעולות (סבב 139) — ⚠️ שם פעולה שחי מחוץ למפה
     *  אינו מנותב, ⭐ והמפה היא נקודת הניתוב האחת. */
    probe: () => /'my-pass':\s*function/.test(bodyOf(src, APP.actMap + ' =')) &&
                 /'switch-user':\s*function/.test(bodyOf(src, APP.actMap + ' =')) },
  /* ⚠️ סוף פר-אפליקציה */
];
