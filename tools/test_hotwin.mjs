...
const APP = {
  ...
  mutations: [
    /*  ⛔ המוטציה שוברת את המנגנון ⛔ ולא את הצורה — ⚠️ היא מכבה את
     *  ההכרעה מול האופק ומחזירה «הכול בחלון», ⭐ ואז שום רשומה אינה
     *  מפונה: ⛔ והטענה על ההכרעה נופלת. */
    ['return d >= hwHorizonDate();',
     'return true;',
     /return d >= hwHorizonDate\(\);/,
     'מוטציה שמבטלת את ההכרעה מול אופק הפינוי נתפסת'],
  ],
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
