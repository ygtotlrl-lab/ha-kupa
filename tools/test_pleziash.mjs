...
const FLOOR = { shared: 0, app: 8, appWhy: 'שרשרת המינימום החודשי והעברת העודף — ⛔ והיא קיימת כאן בלבד: ⚠️ בשאר היעד נמדד ונגמר בחודשו' };
...
function checks(chain) {
  ...
  return [
    ...
    ['ד · חודש בגירעון אינו מוריש חוב', Math.round(c[1].carryOut * 100), 0, `חסר ${Math.round(c[1].short)}`],
    ['ה · העודף עובר במלואו לחודש הבא', badLink.length, 0, `עודף ${Math.round(c[0].carryOut)}`],
    ['ו · גירעון שני נמדד מול המינימום שלו בלבד', Math.round(c[2].short * 100), Math.round((P - 100) * 100)],
