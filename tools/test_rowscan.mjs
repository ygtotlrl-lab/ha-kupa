...
const APP = {
  ...
  gateNoRows: {
    ...
    'test_chumash.mjs':            'יתרת החומש המצטברת — ⚠️ הכלל הוא של המוצר ולא של התשתית, ⛔ ואין לו שורה בטבלה: ⭐ הטבלה מודדת תשתית ⛔ ולא חשבון של מוצר',
    'test_pleziash.mjs':           'שרשרת המינימום החודשי — ⚠️ הכלל הוא של המוצר ולא של התשתית, ⛔ ואין לו שורה בטבלה: ⭐ הטבלה מודדת תשתית ⛔ ולא חשבון של מוצר',
    'test_so_instances.mjs':       'מופע הוראת הקבע — ⚠️ יכולת מוצר שקיימת כאן בלבד, ⛔ ואין לה שורה בטבלה: ⭐ הטבלה מודדת תשתית ⛔ ולא מודל הנתונים של מוצר יחיד',
    'test_so_version.mjs':         'פתיחת גרסה להוראת קבע — ⚠️ יכולת מוצר שקיימת כאן בלבד, ⛔ ואין לה שורה בטבלה: ⭐ הטבלה מודדת תשתית ⛔ ולא מודל הנתונים של מוצר יחיד',
    'test_rasp.mjs':               'אסימון הזהות ושני אתריו — ⚠️ הכלל הוא של עיצוב המוצר, ⛔ והשורה שבטבלה מודדת את אוצר המילים המשותף ⛔ ולא את אסימון המוצר',
    'test_month_open.mjs':         'השומר שלפני סגירת חודש — ⚠️ יכולת מוצר שקיימת כאן בלבד, ⛔ ואין לה שורה בטבלה: ⭐ הטבלה מודדת תשתית ⛔ ולא מודל הנתונים של מוצר יחיד',
    'test_sums.mjs':             'אף סיכום אינו נשמר — ⚠️ הכלל הוא של המוצר ולא של התשתית, ⛔ ואין לו שורה בטבלה: ⭐ הטבלה מודדת תשתית ⛔ ולא את מודל הנתונים של מוצר יחיד',
    'test_examples.mjs':         'שלוש הדוגמאות המספריות מול ליבת החישוב — ⚠️ הטענה היא שהמספר שיצא הוא המספר שנמסר, ⛔ ואין לה שורה: ⭐ הטבלה מודדת תשתית ⛔ ולא חשבון של מוצר',
