...
const APP = {
  ...
  evictCalc: {
    seeds: ['chainAll', 'mirrorLoad'],
    /*  ⛔ הקוראים — ⚠️ פונקציה שנוקבת באחד מהם ומסכמת נמדדת, ⭐ או נכשלת. */
    readers: ['MIRROR.k_entries', 'MIRROR.k_so_instances'],
    seed: `(function () {
  var DAY = 86400000, now = Date.now(), iso = function (t) { return new Date(t).toISOString().slice(0, 10); }, ent = [];
  for (var d = 900; d >= 0; d -= 11) {
    ent.push({ client_id: 'i' + d, type: 'income', amount: 1000 + d, entry_date: iso(now - d * DAY), category: 'x', method: 'm', updated_at: now - d * DAY, deleted: false });
    ent.push({ client_id: 't' + d, type: 'tzedakah', amount: 80 + (d % 7), entry_date: iso(now - d * DAY), category: 'self', method: 'm', updated_at: now - d * DAY, deleted: false });
  }
  localStorage.setItem('k_mirror_entries', JSON.stringify(ent));
  localStorage.setItem('k_mirror_pledges', JSON.stringify([{ hebrew_year: 5786, pledge: 300, chumash_opening_balance: 0, updated_at: now - 500 * DAY, deleted: false }]));
})();`,
    load: "mirrorLoad();",
    calls: {
      chainAll: 'chainAll()',
  ...
  /*  ⛔ מוטציות פרטיות לאפליקציה — ⚠️ **מה נכנס**: מוטציה למנגנון פינוי שקיים
   *  כאן בלבד; ⛔ **ומה מפיל**: מוטציה שאינה מפילה את הטענה שהיא נוקבת בה —
   *  ⭐ והמוטציות המשותפות חלות גם כאן. */
  mutations: [
    ['  oldRecords: [],', "  oldRecords: [{ key: mirrorKey('k_entries'), ts: MIRROR_CFG.ts, syncedThrough: function () { return 0; } }],",
     '[evict-calc-stable]', 'התנועות שחזרו לפינוי'],
  ],
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
