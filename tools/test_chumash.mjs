...
const FLOOR = { shared: 0, app: 10, appWhy: 'יתרת החומש המצטברת חוצת-השנים — ⛔ והיא קיימת כאן בלבד: ⚠️ אין בשאר מספר שעובר מסוף שנה לתחילת הבאה' };
...
const MONTHS = [
  ...
  /*  ⛔ חודש בלי רשומות — ⚠️ הוא מצב ההתקנה הטרייה, ⭐ ושרשרת שנבדקה
   *  רק על חודשים מלאים עוברת בדיוק במקום שבו היא הכי עלולה להישבר. */
  { key: 'כסלו-5787',  year: 5787, rows: [] },
];
const OPENING = 500;
...
function checks(ctx) {
  ...
  return [
    ...
    ['ט · חודש בלי רשומות אינו משנה את היתרה', Math.round(c[4].balance * 100), Math.round(c[3].balance * 100)],
    ['י · ויתרת הפתיחה שלו היא יתרת החודש שלפניו', Math.round(c[4].prevBalance * 100), Math.round(c[3].balance * 100)],
  ];
}
