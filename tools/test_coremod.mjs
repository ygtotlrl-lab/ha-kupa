...
const APP = {
  ...
  coreAllow: {
    hwHorizonDate:
      'גוזרת את תאריך אופק החלון החם מחודש הקופה — ⚠️ החודש הוא חודש עברי, ⛔ ואין לו מקבילה באפליקציה שמונה לועזי',
    mergeTable:
      'מזגת טבלה שלמה של הקופה מעל הליבה — ⚠️ היא בוחרת את משווה החותמות לפי הטבלה, ⛔ והליבה עצמה חיה במודול',
    mergeRows:
      'העוטף הפר-אפליקציתי שמעל `mergeCore` לשורות הקופה — ⚠️ הוא קובע את מפתח הזהות ואת סינון המחוקים, ⛔ והליבה עצמה חיה במודול',
    pushSoon:
      'קובעת מועד דחיפה מושהה אחרי כתיבה מקומית בקופה — ⚠️ היא נקראת מכל מסלול שמירה כאן, ⛔ והמתזמן המשותף חי בשכבת הדחיפה',
    hebLabel:
      'ממירה תאריך ISO ליום מעוגן בצהריים ומוסרת אותו לצרכן התצוגה — ⚠️ הרשומה כאן נושאת את התאריך כמחרוזת, ⛔ ובשתי האחרות הצרכן מקבל `Date` שנבנה באתר הקריאה: ⭐ והתווית עצמה נגזרת מהמודול',
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
