...
const APP = {
  ...
  appFns: {
    ...
    mergeRows:
      'עוטפת את ליבת המיזוג המשותפת בשלוש ידיות המדיניות של שורות טבלה — מפתח שהוא שם עמודה, כפילות שנשארת גלויה, וההתאמה הראשונה כמתמודדת מול הענן — ⛔ וביומן, בהנהלה ובשכר המיזוג הוא פר-רשומה ולא פר-שורת טבלה, והידיות שלהן נבדלות',
    _hcHTable:
      'מחזירה את תאריך הלוח העברי מטבלה אריתמטית קבועה, כנפילה-חזרה למנוע התאריך כשהדפדפן אינו נושא לוח עברי — ⛔ ובשכר ובגיוס אפס צרכני תאריך עברי, ואין ערך שיוצג',
    ysHebDate:
      'מחזירה את התאריך העברי לתצוגה מעל הלוח המובנה בדפדפן, ובנפילה-חזרה מהטבלה האריתמטית — ⛔ ובשכר ובגיוס אין תאריך עברי על המסך ואין צרכן שיקרא לה',
    _ysBadDate:
      'רושמת ביומן תאריך פסול שהגיע למנוע, עם אתר הקריאה — ⛔ ובשכר ובגיוס אפס צרכני תאריך עברי, ואין ערך שיוצג',
    _ysHebNone:
      'מחזירה את הערך הריק המוסכם כשאין תאריך עברי להחזיר — ⛔ ובשכר ובגיוס אפס צרכני תאריך עברי, ואין ערך שיוצג',
    _ysIsDate:
      'מכריעה אם הערך שהתקבל הוא תאריך תקין לפני ההמרה — ⛔ ובשכר ובגיוס אפס צרכני תאריך עברי, ואין ערך שיוצג',
    _ysMonthCode:
      'ממפה את שם החודש שהלוח המובנה החזיר לקוד החודש הפנימי — ⛔ ובשכר ובגיוס אפס צרכני תאריך עברי, ואין ערך שיוצג',
    ysGematria:
      'ממירה מספר לאותיות עבריות עם הגרש במקומו — ⛔ ובשכר ובגיוס אפס צרכני תאריך עברי, ואין ערך שיוצג',
    ysHebDayLabel:
      'מחזירה את תווית היום בחודש העברי מטבלת הימים — ⛔ ובשכר ובגיוס אפס צרכני תאריך עברי, ואין ערך שיוצג',
    ysHebIsLeap:
      'מכריעה אם שנה עברית מעוברת, לצורך «אדר א» ו«אדר ב» הנפרדים — ⛔ ובשכר ובגיוס אפס צרכני תאריך עברי, ואין ערך שיוצג',
    ysHebMonthNames:
      'מחזירה את טבלת שמות החודשים המתאימה לשנה פשוטה או מעוברת — ⛔ ובשכר ובגיוס אפס צרכני תאריך עברי, ואין ערך שיוצג',
    ysHebYearLabelFull:
      'מחזירה את תווית השנה העברית עם האלפים במלואם — ⛔ ובשכר ובגיוס אפס צרכני תאריך עברי, ואין ערך שיוצג',
    hebrewDate:
      'מעצבת תאריך עברי לתצוגה מעל מנוע התאריך המשותף — ⛔ ובשכר ובגיוס אפס צרכני תאריך עברי, ואין ערך שיוצג',
    ysIntlHeb:
      'מעצבת תאריך בלוח העברי המובנה בדפדפן ומחזירה את חלקיו — ⛔ ובשכר ובגיוס אפס צרכני תאריך עברי, ואין ערך שיוצג',
    bar:
  ...
  productFns: {
    ...
    mergeRows:
      'ממזגת שורות טבלה מעל מנוע המיזוג המשותף — ⚠️ ומקור החותמת נבדל: בגיוס יש נפילה-חזרה ל-`created_at` ב-ISO, ובקופה החותמת מספרית בכל שורה',
    saveRefresh:
