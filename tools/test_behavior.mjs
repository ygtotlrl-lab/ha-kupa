...
const APP = {
  ...
  ksaveOpen:
    "(function () { var a = document.querySelector('#modal.open [data-act=\"add-tzedakah\"]'); " +
             "if (a) { a.click(); return 1; } " +
             "var f = document.getElementById('fab'); if (f) { f.click(); return 0; } return -1; })()",
   ...
   *  לה כלל. */
  dragHandles: [
    { sel: '.grip', why: 'ידית שורת הרשימה — ממנה נקבע סדר האופנים' },
    { sel: '.coin', why: 'המטבע עצמו הוא הידית — הוא נגרר אל המטרה, ואין בו אזור שאינו נגרר' },
  ],
   ...
   *  ⛔ והגרירה עצמה אחת. */
  drag: {
    expose: ['MIRROR', 'view', 'kRender'],
    setup: "(function () { var M = window.__acc.MIRROR;" +
      " if (!M.k_lookups) M.k_lookups = [];" +
      " if (!M.k_lookups.length) [1, 2, 3].forEach(function (i) {" +
      "   M.k_lookups.push({ client_id: 'zz' + i, kind: 'source'," +
      "     label: '\u05d0\u05d5\u05e4\u05df ' + i, sort: i, deleted: false, updated_at: 1 }); });" +
      " window.__acc.view.screen = 'settings'; window.__acc.view.lookOpen = 'source';" +
      " window.__acc.kRender();" +
      " return document.querySelectorAll('.det.drag[data-kind=\"source\"] > li').length === 3; })()",
    grip: '.det.drag[data-kind="source"] > li:nth-of-type(1) > .grip',
    target: '.det.drag[data-kind="source"] > li:nth-of-type(2)',
    order: "[].map.call(document.querySelectorAll('.det.drag[data-kind=\"source\"] > li .nm'), function (e) { return e.textContent; }).join(',')",
    /*  ⛔ המוטציה משאירה את הציור ומסירה את הכתיבה — ⚠️ העץ זז
     *  בזמן הגרירה מעצמו, ⭐ ומוטציה שמסירה גם את הציור משאירה
     *  אותו זז ⛔ ונראית כהצלחה. */
    mut: ['if (lookupReorder(kind, ids)) kRender();', 'kRender();'],
  },
  dragWhy: '',
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
