...
const APP = {
  ...
  calendar: { kind: 'hebrew',
    why: 'החומש והפלעדזש נמנים לחודש עברי — ⚠️ הצדקה נמדדת מא׳ דחודש, ⭐ והיתרה מתגלגלת בתשרי' },
   ...
   *  סבב. ⭐ **ולמה היא קיימת**: התחילית הוסבה בסבב 148, ⛔ וההגירה היא
   *  המקום היחיד בקוד החי שנוקב בישנה — ⚠️ בלעדיה מפתחות האחסון הישנים
   *  אבודים, ⛔ ואיתם התור שלא נדחף. */
  prefixLegacy: {
    'kp_': { fn: 'kpMigrateLegacyPrefix',
      why: 'התחילית שירדה בהסבה לראשי התיבות של השם העברי — ⚠️ וההגירה מעבירה כל מפתח באחסון המקומי, מלא או בנוי מקידומת (סבב 148)' },
  },
  ...
  coreVerbs: {
    monthKeyOf: { name: 'monthKeyOf',
      why: 'מפתח החודש של רשומה — ⚠️ וכאן הוא שנה עברית ומספר החודש בתוכה, ⭐ נגזר מהמנוע מתאריך הרשומה' },
    monthLabel: { name: 'monthLabel',
      why: 'תווית החודש לתצוגה — ⚠️ וכאן היא שם החודש העברי, ⭐ שנגזר מטבלת השמות של אותה שנה' },
    monthsWithData: { name: 'monthsWithData',
      why: 'החודשים שיש בהם רשומות — ⚠️ והחודש הנוכחי נכנס תמיד, ⭐ שבלעדיו התקנה טרייה נפתחת למסך שלד' },
    yearOf: { name: 'yearOf',
      why: 'השנה של רשומה — ⚠️ וכאן היא השנה העברית, ⭐ שהפלעדזש מוצמד אליה' },
  },
};
/* ── סוף APP ───────────────────────────────────────────────────────────── */
