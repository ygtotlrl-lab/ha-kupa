...
const APP = {
  ...
  appGates: {
      'sums': 'מודד שאף סיכום אינו נשמר — ⛔ אין עמודת צבירה, אין `.rpc(` ואין `view`: ⚠️ ולשאר אין שרשרת חודשים מצטברת שסיכום שמור היה מקצר',
      'chumash': 'מודד שיתרת החומש מצטברת חוצה שנים ושיתרת פתיחה מוצהרת לשנה הראשונה בלבד — ⛔ ולשאר אין מספר שעובר מסוף שנה לתחילת הבאה',
      'pleziash': 'מודד שעודף המינימום החודשי עובר לחודש הבא וחוב אינו עובר — ⛔ ולשאר היעד החודשי נמדד ונגמר בחודשו',
      'so_instances': 'מודד שמופע הוראת קבע נספר בצדקה מרגע שנוצר ולא ביום התשלום — ⛔ ולשאר החיוב החודשי נגזר משדה ואינו רשומה',
      'so_version': 'מודד שעריכת הוראת קבע פותחת שורה חדשה חסומה בחודשים ואינה משנה מופעים שנספרו — ⛔ ולשאר אין הוראה שתחולתה חסומה',
      'rasp': 'מודד שאסימון הזהות מיושם בשני כללי CSS בלבד ואינו נכתב כמחרוזת CSS מ-JS — ⛔ ולשאר צבע הזהות נצבע על כל רכיב מייצג',
      'month_open': 'מודד שכל אתר סגירת חודש עטוף בשומר שבודק שהחסר התאפס — ⛔ ולשאר התקופה היא טווח בשאילתה ואין רשומת חודש שנסגרת',
      'examples': 'מודד את שלוש הדוגמאות המספריות מול ליבת החישוב — ⛔ ולשאר אין שרשרת חודשים שהחומש, היתרה והמילוי נגזרים ממנה',
  },
  only: {
    'tools/kp_calc.mjs': 'ליבת חישוב החומש, היתרה והמילוי — ⛔ אף סיכום אינו נשמר, ⚠️ והכל נגזר מהרישומים בזמן קריאה; ⭐ ולשאר אין שרשרת חודשים שנגזרת',
